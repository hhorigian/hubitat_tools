...
hhorigian - 28.2.2026 - adjusted settings for token and open/close of WS. 
===========================================================================================*/
...
def connect(funct) { // library marker davegut.samsungTvWebsocket, line 240
	...
	state.lastFunction = funct // TRATO: remember last valid websocket function
...
// ===== TRATO minimal stability patch for Samsung 2024/2025 =====
def reconnectWsAfterError() {
	def funct = (state.reconnectFunction ?: (state.lastFunction ?: "remote"))
	if (!(funct in ["remote","frameArt"])) { funct = "remote" }
	logDebug("reconnectWsAfterError: reconnecting function=${funct}")
	connect(funct)
}
...
def parse(resp) { // library marker davegut.samsungTvWebsocket, line 295
	...
	try { // library marker davegut.samsungTvWebsocket, line 297
		...
		switch(event) { // library marker davegut.samsungTvWebsocket, line 301
			...
			case "ms.error": // TRATO: reconnect on websocket error without clearing token
				def sw = device.currentValue("switch")
				if (sw != "on") {
					// If TV is off/standby, don't thrash reconnect; wake is handled via WOL in on()
					logDebug("parse: EVENT ms.error while switch=${sw} - closing websocket (no reconnect)")
					close()
					break
				}
				logWarn("parse: EVENT ms.error - reconnecting websocket")
				close()
				state.reconnectFunction = (state.lastFunction in ["remote","frameArt"] ? state.lastFunction : "remote")
				runInMillis(1500, "reconnectWsAfterError")
				break
